...
const Register: React.FC = () => {
    ...
    const handleGoogleRegister = async () => {
        setLoading(true);
        try {
            const { firebaseUser, userData } = await loginWithGoogle();
            if (userData) {
                showToast(`Xavfsiz hisob topildi. Xush kelibsiz! 👋`, 'success');
                navigate('/dashboard');
                return;
            }
            ...
            setPassword('google-auth-protected'); // dummy password for form validation if needed
            setConfirmPassword('google-auth-protected');
            setIsGoogleSignup(true);
            showToast("Iltimos, maktab ma'lumotlarini kiriting", 'info');
        } catch (error: any) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            await register({
                name,
                email,
                password: isGoogleSignup ? '' : password, // auth service will handle social login if needed, or we just rely on the fact they are logged in.
                // Wait, registerUser in auth.service.ts uses createUserWithEmailAndPassword.
                // I need to update registerUser to handle Google users who just need DB entry.
                sinf,
                viloyat,
                tuman,
                maktab,
                ...
                <div className="bg-white rounded-[2rem] overflow-hidden">
                    ...
                    {phase === 1 && (
                        <form onSubmit={handlePhase1Submit} className="px-8 pb-8 animate-fade-in">
                            ...
                            {/* Passwords - Hide if Google signup */}
                            {!isGoogleSignup && (
                                <>
                                    <div className="relative mb-4">
                                        <input
                            ...
                            {!isGoogleSignup && (
                                <>
                                    ...
                                    <button
                                        ...
                                        disabled={loading}
                                        className="w-full py-4 bg-white border-2 border-gray-100 text-gray-700 rounded-2xl font-semibold text-lg flex items-center justify-center gap-3 hover:bg-gray-50 transition disabled:opacity-50"
                                    >
